defmodule DataGeneratorWeb.EnumsLive.Edit do
  ...
  def mount(%{"id" => id}, _session, socket) do
    ...
    {values, next_id} = build_values_from_enum(enum)

    {:ok,
     assign(socket,
       page_title: "Edit Enum",
       enum: enum,
    ...
    <Layouts.app flash={@flash} current_scope={@current_scope}>
      <div class="space-y-6 max-w-xl">
        <div>
          ...
          <h1 class="text-2xl font-bold text-gray-900">Edit Enum</h1>
          <p class="mt-1 text-sm text-gray-600">Update the name and values for this enum.</p>
        </div>
        ...
        >
          ...
          <div class="rounded-xl border border-gray-200 bg-white shadow-sm">
            ...
            <div class="divide-y divide-gray-100">
              <%= for val <- @values do %>
                <div class="flex items-center gap-3 px-6 py-3" id={"edit-value-#{val.temp_id}"}>
                  <input
                    type="text"
  ...
  def handle_event("add_value", _params, socket) do
    ...
    temp_id = "val-#{id}"
    values = socket.assigns.values ++ [%{temp_id: temp_id, value: "", db_id: nil}]
  ...
  def handle_event("save", %{"enum" => enum_params} = params, socket) do
    enum = socket.assigns.enum
    values = sync_values_from_params(params, socket.assigns.values)

    enum_values =
      values
      |> Elixir.Enum.reject(&(&1.value == ""))
      |> Elixir.Enum.map(fn val ->
        base = %{"value" => val.value}
        if val.db_id, do: Map.put(base, "id", val.db_id), else: base
      end)

    attrs = Map.put(enum_params, "enum_values", enum_values)

    case Enums.update_enum(enum, attrs) do
      {:ok, _enum} ->
        {:noreply,
         socket
         |> put_flash(:info, "Enum updated successfully.")
  ...
  defp build_values_from_enum(enum) do
    values =
      enum.enum_values
      |> Elixir.Enum.with_index()
      |> Elixir.Enum.map(fn {ev, idx} ->
        %{temp_id: "val-#{idx}", value: ev.value, db_id: ev.id}
      end)

    values = if values == [], do: [%{temp_id: "val-0", value: "", db_id: nil}], else: values
    next_id = length(values)
    {values, next_id}
  end
  ...
  defp build_changeset(socket, enum_params, values) do
    ...
    Enums.change_enum(socket.assigns.enum, attrs)
  end
end
